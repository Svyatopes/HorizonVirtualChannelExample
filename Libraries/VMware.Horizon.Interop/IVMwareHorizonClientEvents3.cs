...
namespace VMware.Horizon.Interop
{
  ...
  public interface IVMwareHorizonClientEvents3 : IVMwareHorizonClientEvents2
  {
    ...
    [MethodImpl(MethodImplOptions.InternalCall, MethodCodeType = MethodCodeType.Runtime)]
    void OnFolderCanBeShared([In] uint serverId, [MarshalAs(UnmanagedType.BStr), In] string sessionToken, [In] uint canShare);

    [MethodImpl(MethodImplOptions.InternalCall, MethodCodeType = MethodCodeType.Runtime)]
    void OnCDRForcedByAgent([In] uint serverId, [MarshalAs(UnmanagedType.BStr), In] string sessionToken, [In] uint forcedByAgent);
  }
}